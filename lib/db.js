...
function byUpdated(view) {
  return (_query) => {
    const query = R.merge(_query, { include_docs: true, reduce: false });
    return db.query(view, query).then(({ rows }) => (
      R.into(
        [], R.compose(R.filter(row => !row.value), R.map(getDocFromRow)), rows
      )
    ));
  };
}
...
const viewHandlers = {
  ...
  by_updated: byUpdated('app/by_updated'),
  published_by_updated: byUpdated('app/published_by_updated'),
};
