...
const Crypto = require('../../lib/crypto.coffee');
const DB = require('../../lib/db.coffee');
const Lang = require('../../lib/lang.coffee');
const logger = require('../../lib/log.coffee');
const sendMail = require('../../lib/send-mail.coffee');
const URL = require('./url-helpers.coffee');
const utils = require('../scripts/utils.coffee');
const { read } = require('../../lib/utils.coffee');
...
function fetchArticles(params) {
  ...
  if (params.year) {
    ...
    const dateParams = R.map(_.parseInt, R.pick(dateKeys, params));
    if (dateParams.month) {
...
function fetchLocaleStrings(_lang) {
  ...
  return readAsync(`locales/${lang}.yaml`)
    .then(text => ({ lang: lang, data: YAML.safeLoad(text) }));
}
