...
function validationReducer(parts, item) {
  const [key, validation] = item;
  const isValid = validate(parts[0], validation);
  if (isValid) {
    return [parts.slice(1), [key, parts[0]]];
  }
  return [parts, null];
}
...
function getParams(arg) {
  ...
  const params = R.fromPairs(
    R.mapAccum(validationReducer, parts, R.zip(keys, validation))[1]
      .filter(Boolean)
  );
  if (arg.userId) {
